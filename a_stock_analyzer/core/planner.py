...
from a_stock_analyzer.core.finding import Finding
from a_stock_analyzer.core.research_plan import ResearchPlan, TaskNode

logger = logging.getLogger(__name__)

# Valid roles for task nodes
VALID_ROLES = {"web_search", "data_fetch", "doc_analysis", "cross_verify", "synthesis"}
...
2. role 只能从以下选择：[web_search, data_fetch, doc_analysis, cross_verify, synthesis]
   - web_search: 通过网络搜索获取最新资讯和观点
   - data_fetch: 从结构化数据源（如AKShare）获取精确数据
...
6. 对于公司财报分析，建议拆分：
   - data_fetch: 获取核心财务数据
   - web_search: 搜索分析师解读和行业背景
   - doc_analysis: 深度分析财务指标变化
  ...
  "tasks": [
    ...
    def _parse_plan(self, content: str, user_query: str) -> ResearchPlan | None:
        ...
        for t_data in data["tasks"]:
            ...
            role = t_data.get("role", "web_search")
    ...
    def _fallback_plan(self, user_query: str) -> ResearchPlan:
        ...
        return ResearchPlan(
            ...
            user_query=user_query,
            tasks=[
                TaskNode(task_id="t1", role="web_search", goal=f"搜索{user_query}相关信息", depends_on=[]),
                TaskNode(task_id="t2", role="data_fetch", goal=f"获取{user_query}相关数据", depends_on=[]),
...
{{"is_complete": false, "reason": "原因", "new_tasks": [
  {{"task_id": "t_new_1", "role": "web_search", "goal": "补充调研目标", "depends_on": []}}
]}}
