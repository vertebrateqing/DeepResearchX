...
[ -d "$BACKEND_DIR" ]  || { echo -e "${R}  ✗ 找不到 backend 目录${N}"; exit 1; }
[ -d "$FRONTEND_DIR" ] || { echo -e "${R}  ✗ 找不到 frontend 目录${N}"; exit 1; }
echo -e "${G}  ✓ 项目结构正常${N}"

# 2. uv
echo -e "${Y}[2/5] 检查 uv 工具链...${N}"
if ! command -v uv &> /dev/null; then
    echo -e "${R}  ✗ 未找到 uv，请先安装：${N}"
    echo -e "${C}      curl -LsSf https://astral.sh/uv/install.sh | sh${N}"
    exit 1
fi
UV_VERSION=$(uv --version 2>&1 | head -1)
echo -e "${G}  ✓ ${UV_VERSION}${N}"

# 3. 同步后端依赖
echo -e "${Y}[3/5] 同步后端依赖（uv sync）...${N}"
cd "$BACKEND_DIR"
if uv sync --quiet; then
    echo -e "${G}  ✓ 后端依赖已就绪${N}"
else
    echo -e "${R}  ✗ uv sync 失败，请检查 pyproject.toml${N}"
    exit 1
fi
cd "$PROJECT_DIR"
...
command -v npm &> /dev/null || { echo -e "${R}  ✗ 未找到 npm，请先安装 Node.js 20+${N}"; exit 1; }
...
if check_port 8000; then
    ...
    (
        cd "$BACKEND_DIR"
        nohup env "${LF_ENV[@]}" uv run uvicorn main:app --reload --host 0.0.0.0 --port 8000 > "$BACKEND_LOG" 2>&1 &
        echo $! > "$BACKEND_PID"
    )
    sleep 2
    if check_port 8000; then
        echo -e "${G}  ✓ 后端已启动  → ${B}http://localhost:8000${N}"
        [ ${#LF_ENV[@]} -gt 0 ] && echo -e "${G}    Langfuse 可观测性  → ${B}http://localhost:3000${N}"
    else
