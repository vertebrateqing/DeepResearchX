...
Re-ingests the PDF, then auto-discovers relevant chunk IDs by searching
the actual chunk text, so the benchmark stays valid regardless of chunking
parameters or embedding provider.
...
# Search terms that should appear in relevant chunks for each query
QUERY_SEARCH_TERMS: dict[str, list[str]] = {
    ...
    "alibaba_010": [],  # negative — no relevant chunks
}


def _find_relevant_chunks(all_chunks: list[dict], terms: list[str]) -> list[str]:
    """Find chunk IDs whose text contains any of the search terms."""
    if not terms:
        return []
    results = []
    for c in all_chunks:
...
async def main() -> int:
    ...
    # 4. Retrieve all chunks for relevance discovery
    print("\nRetrieving all chunks for relevance discovery...")
    ...
    # 5. Auto-discover relevant chunk IDs
    print("\nAuto-discovering relevant chunks...")
    updated_cases = []
    with open(benchmark_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            print(f"  {qid}: found {len(relevant_ids)} relevant chunks (terms: {terms})")
    ...
    print(f"\nUpdated benchmark written to: {updated_path}")
