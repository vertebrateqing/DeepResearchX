...
from financial_agent.rag.document_loader import Document, PDFDocumentLoader
from financial_agent.rag.embedding import EmbeddingService
from financial_agent.rag.hybrid_retriever import HybridRetriever
from financial_agent.rag.query_rewriter import QueryRewriter
from financial_agent.rag.reranker import CrossEncoderReranker
...
def _merge_retrieval_results(
    ...
    for results in all_results:
        for doc in results:
            ...
            if doc_id in seen:
                # Keep the better score (higher similarity = better)
    ...
    merged = sorted(seen.values(), key=lambda d: d.get("score", 0), reverse=True)
    return merged
