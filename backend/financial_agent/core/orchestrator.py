...
class OrchestratorAgent(BaseAgent):
    ...
    async def _pre_search_chapters(
        ...
        for ch in chapters:
            ...
            # Emit sources event for this chapter
    ...
    async def _execute_chapters(
        ...
        self._emit("status", {"message": f"已完成 {len(chapter_files)} 个章节的撰写", "stage": "chapters", "progress": 45})

        # --- Step 3: Run revision loops in parallel ---
        self._emit("status", {"message": "正在审校各章节质量...", "stage": "chapters", "progress": 50})
