        ...
        for slot in self.missing_slots:
            if slot.confirmed and slot.extracted_value:
                ...
                if slot.slot_name == "report_year":
                    ...
                    merged = f"请研究{val}：{merged}"
                elif slot.slot_name == "top_n":
                    merged = f"{merged}（数量：{val}）"
                else:
...
class IntentClarifier:
    ...
    async def _llm_based_detection(self, query: str) -> list[MissingSlot]:
        ...
        has_research_keyword = any(kw in stripped for kw in [
            "分析", "研究", "调查", "报告", "综述", "比较", "评估", "趋势",
            "介绍", "了解", "探讨", "总结", "梳理", "解读", "概述",
            "行业", "公司", "技术", "产品", "市场", "政策", "科技",
        ])
        has_temporal = any(kw in stripped for kw in [
            "20", "年", "季度", "Q1", "Q2", "Q3", "Q4", "去年", "今年", "明年", "最近", "最新", "一期",
        ])
        # If query is very short AND lacks both research keywords and temporal info, it's likely vague
        if len(stripped) < 8 and not has_research_keyword and not has_temporal:
            logger.info(f"[IntentClarifier] Query '{stripped}' is too vague, forcing clarification")
            return [
                MissingSlot(
                    slot_name="analysis_target",
                    slot_type="string",
                    question="您想研究或分析什么主题？",
                    default_value="",
                ),
            ]
...
- 如果用户已经明确提供了某个信息（如年份、主题名称、数量等），即使该信息在时间上不合理或与你所知不符，也不要视为缺失
- **名称歧义判断标准**：只有简称确实可能指代多个**不同**对象时才视为歧义。对于市场知名度极高、几乎无歧义的名称，不算歧义，不要返回。
- 如果用户请求中包含"202X年"等明确时间信息，不要询问年份。
- **研究目标缺失判断标准**：如果用户请求没有提到任何具体的研究对象（如没提主题、领域、名称等），只说了"帮我看看""分析一下""研究一下"这类泛泛的话，必须视为缺失，返回 analysis_target。

请检查以下方面：
1. 是否缺少研究目标/主题（如用户只说"分析一下""帮我看看"，没有任何具体对象）
2. 是否完全缺少时间信息（当研究主题有时效性要求时）
3. 名称是否可能有歧义（如简称可能对应多个不同对象）
4. 研究范围或深度是否未明确
5. 数量要求是否未明确

【示例】
- 用户"帮我看看" → 缺少研究目标，返回 analysis_target
- 用户"分析一下" → 缺少研究目标，返回 analysis_target
- 用户"分析一下人工智能行业发展趋势" → 无缺失，返回 []
- 用户"研究新能源汽车市场" → 无缺失（目标明确），返回 []
