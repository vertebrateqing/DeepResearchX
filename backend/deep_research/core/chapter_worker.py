...
# LLM never hallucinates a call to a tool that isn't registered.
_REACT_TOOL_HINTS: dict[str, str] = {
    "document_search": "- document_search: 在用户上传的本地文档（PDF / Word / 文本）中检索最相关段落，优先使用",
...
def _build_react_system_prompt(tools: list[BaseTool]) -> str:
    """Compose the reviser system prompt from whichever tools are actually registered.

    Keeping the tool list and prompt in sync prevents the LLM from being told
    to call a tool that ``ReActAgent`` can't dispatch (which raises
    ``Tool not found in agent ...``).
    """
    tool_names = {getattr(t, "name", "") for t in tools}
    tool_lines = [hint for name, hint in _REACT_TOOL_HINTS.items() if name in tool_names]

    sections = [REACT_SYSTEM_PROMPT_HEADER]
    if tool_lines:
        sections.append("\n【可用工具】\n" + "\n".join(tool_lines))
        if "document_search" in tool_names:
            sections.append("优先调用 document_search 获取用户上传文档中的素材；联网搜索作为补充。")
    sections.append("\n" + REACT_SYSTEM_PROMPT_BODY)
    return "\n".join(sections)
    ...
    async def revise(self, feedback: str) -> None:
        ...
        try:
            react_agent = ReActAgent(
                ...
                system_prompt=_build_react_system_prompt(self.tools),
                tools=self.tools,
                skills=self.skills,
                model=self.model,
