...
def _tokenize(text: str) -> list[str]:
    """Tokenize text with language-aware strategy: jieba for Chinese, regex for others."""
    import re
    try:
...
def bm25_scores(query: str, texts: list[str]) -> list[float]:
    """Compute BM25 relevance scores for query against a list of texts."""
...
def rrf_fusion(
    ...
    """Reciprocal Rank Fusion of vector and BM25 score lists."""
    n = len(vector_scores)

    vec_ranks = [0] * n
    for rank, idx in enumerate(sorted(range(n), key=lambda i: vector_scores[i], reverse=True)):
        vec_ranks[idx] = rank

    bm25_ranks = [0] * n
    for rank, idx in enumerate(sorted(range(n), key=lambda i: bm25_scores_list[i], reverse=True)):
        bm25_ranks[idx] = rank

    return [1.0 / (k + vec_ranks[i] + 1) + 1.0 / (k + bm25_ranks[i] + 1) for i in range(n)]
...
class WebScraperTool(BaseTool):
    ...
    async def execute(
        ...
        # Filter by hybrid BM25 + vector similarity.
        # If chunks exceed the embedding threshold, use BM25 to pre-filter down to
        # a manageable size first, then apply full hybrid scoring on the shortlist.
        ...
        if all_chunks and query:
            if len(all_chunks) > max_chunks_for_embedding:
                logger.info(
                    ...
                    f"BM25 pre-filtering to {max_chunks_for_embedding} before hybrid scoring"
                )
                # BM25 pre-filter: CPU-only, no API calls
                bm25_pre = await asyncio.to_thread(
                    bm25_scores, query, [c.text for c in all_chunks]
                )
                ranked = sorted(zip(bm25_pre, all_chunks), key=lambda x: x[0], reverse=True)
                all_chunks = [c for _, c in ranked[:max_chunks_for_embedding]]

            filtered_chunks = await self._filter_by_hybrid(all_chunks, query, top_k)
        else:
    ...
    # --- Hybrid filtering (BM25 + vector RRF) ---

    async def _filter_by_hybrid(
        self,
        chunks: list[TextChunk],
        query: str,
        top_k: int,
    ) -> list[TextChunk]:
        """Filter chunks using BM25 + vector cosine similarity via RRF fusion."""
        if not chunks:
            return []
        ...
        # BM25 scores — CPU-bound, run in thread
        bm25_list = await asyncio.to_thread(bm25_scores, query, chunk_texts)
        ...
        vec_list = [cosine_similarity(query_embedding, emb) for emb in all_embeddings]

        # RRF fusion and sort
        combined = rrf_fusion(vec_list, bm25_list)
        scored = sorted(zip(combined, chunks), key=lambda x: x[0], reverse=True)

        latency = time.perf_counter() - t0
        logger.info(
            f"[WebScraper] Hybrid filter (BM25+vec RRF): {len(chunks)} chunks in {latency:.2f}s, "
