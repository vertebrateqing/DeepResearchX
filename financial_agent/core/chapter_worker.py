...
from financial_agent.core.base import BaseSkill, BaseTool
...
def _build_chapter_system_prompt(outline: ChapterOutline) -> str:
    ...
    return f"""你是一位专业的投研分析师，负责撰写分析报告的一个独立章节。
...
def _build_react_system_prompt(outline: ChapterOutline) -> str:
    ...
    return f"""你是一位专业的投研分析师，负责修订分析报告的一个独立章节。
...
【可用工具】
建议工具: {tools_hint}
- web_search: 搜索互联网获取最新市场信息、行业动态、公司新闻
- data_fetch: 从结构化数据源（AKShare）获取精确财务数据
- doc_analysis: 分析文档/财报/研报中的关键信息
...
class ChapterWorker:
    ...
    async def execute(self, research_context: str = "") -> Finding:
        """Write chapter using pre-fetched research context (single-shot LLM).

        Args:
            research_context: Pre-searched research material for this chapter.
        ...
        prompt_parts.append(
            f"\n\n请基于以上资料撰写 '{self.outline.title}' 章节。"
            f"目标字数约 {self.outline.word_count} 字。"
        )
        ...
        logger.info(
            ...
            f"'{self.outline.title}', context_len={len(research_context)}"
        )
        ...
        # Build finding
        sources = _extract_sources_from_text(chapter_text)
...
def _filter_tools(
    ...
    if "web_search" in name_map and "web_search" not in suggested:
        filtered.append(name_map["web_search"])
